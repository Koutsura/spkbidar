        ...
        <form action="{{ route('mahasiswa.pendaftaran.submit') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="organization_1" class="form-select" required>
                    ...
                    @foreach($ukms as $ukm)
                        <option value="{{ $ukm }}" {{ old('organization_1', $pendaftaran->organization_1 ?? '') == $ukm ? 'selected' : '' }}>{{ $ukm }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="organization_2" class="form-select">
                    ...
                    @foreach($ukms as $ukm)
                        <option value="{{ $ukm }}" {{ old('organization_2', $pendaftaran->organization_2 ?? '') == $ukm ? 'selected' : '' }}>{{ $ukm }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="organization_3" class="form-select">
                    ...
                    @foreach($ukms as $ukm)
                        <option value="{{ $ukm }}" {{ old('organization_3', $pendaftaran->organization_3 ?? '') == $ukm ? 'selected' : '' }}>{{ $ukm }}</option>
                    @endforeach
                </select>
            </div>
